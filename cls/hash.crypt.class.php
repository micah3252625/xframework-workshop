...
class hash_encryption
{
	 ...
	 * @public	bool	true = use base64, false = binary output / input
	 ...
	 * @public	string	Change this value to add more randomness to your encryption

	 **/

	public $salt = 'Change this to any secret value you like. "d41d8cd98f00b204e9800998ecf8427e" might be a good example.';
	...
	function encrypt($string)
	{
		...
		for ($c = 0; $c < $this->hash_length; $c++) {
			$out .= chr(ord($iv[$c] ?? 0) ^ ord($this->hash_key[$c] ?? 0));
		}
		...
		while ($c < strlen($string)) {
			...
			$out .= chr(ord($key[$c % $this->hash_length] ?? 0) ^ ord($string[$c] ?? 0));
			$c++;
		}
	...
	function decrypt($string)
	{
		...
		for ($c = 0; $c < $this->hash_length; $c++) {
			if (!isset($tmp_iv[$c]) || !isset($this->hash_key[$c])) {
				continue;
			}
	...
	{
		$hash = hash('sha256', $string); // Use hash() function with 'sha256' algorithm
