...
class custom
{
	...
	function new_id($strprefix = "")
	{
		$unique_id = strtoupper(uniqid($strprefix));
		return $unique_id;
	}
	...
	function checkbox($option = "", $value = "", $name = "", $id = "", $selected_value = 0, $other_attributes = "", $separator = "")
	{
		$output = '';
	...
	function dropdown_rs($rs, $value_display = array(), $name = "", $selected_value = "", $select_text = "", $other_attributes = "")
	{
		...
		if ($rs) {
			...
			$str_display = $value_display['display'];
			foreach ($rs as $row) {
				$select_option = "";
	...
	function typeahead_data($rs, $strfield)
	{
		$typeahead_data = '';
		$typeahead_data = '[';
		foreach ($rs as $row) {
			$typeahead_data .= '"' . $this->sql_safe($row->$strfield) . '",';
		}
		if ($typeahead_data == '[') {
			$typeahead_data = '';
		} else {
			$typeahead_data  = substr($typeahead_data, 0, -1);
			$typeahead_data .= ']';
		}
